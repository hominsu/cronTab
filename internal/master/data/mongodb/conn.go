...
func InitMongodbConn(ctx context.Context) error {
	...
	Cli, err = mongo.Connect(ctx,
		options.Client().
			ApplyURI(master_conf.GConfig.MongodbUri).
			SetConnectTimeout(sync.ShrinkDeadLine(ctx, time.Duration(master_conf.GConfig.MongodbConnectTimeout)*time.Millisecond)))
	if err != nil {
