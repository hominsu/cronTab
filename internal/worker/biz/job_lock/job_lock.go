...
func (jobLock *JobLock) TryLock() error {
	...
	keepRespChan, err := jobLock.lease.KeepAlive(ctxLease, leaseId)
	if err != nil {
	...
	// 锁路径
	lockKey := constants.JobLockDir + jobLock.JobName
	...
	if !txnResp.Succeeded {
		...
		return terrors.Wrap(xerrors.ErrorLockAlreadyRequired, "can not get the lock")
	}
